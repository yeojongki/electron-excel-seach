...
import { setImportHistory } from '../utils/storage';
    ...
    >
      <template #bodyCell="{ column, index }">
        <template v-if="column.dataIndex !== 'action'">
          <a-input
            v-model:value="(state.dataSource as any)[index][column.dataIndex]"
          />
        </template>
        ...
        <template v-else>
          <a-tooltip>
            ...
            <ImportOutlined
              class="text-blue icon-text"
              @click="importSelectedTable([state.dataSource[index]])"
            />
          </a-tooltip>
          <!-- <a-button class="auto-height-link-btn" type="link"> </a-button> -->
        </template>
      </template>
    </a-table>
  </a-spin>
